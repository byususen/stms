...
class stms:
    ...
    def spatiotemporal_filling(
        ...
        id_nested : array-like, optional
            Higher-level ID that nests id_sample (e.g., ID_SEGMEN for ID_SUBSEGMEN).
            If provided, we can control how many candidates per nested group (segmen) are chosen.
        candidate_sampling : {"distance", "random"}, optional
            Strategy to order candidate samples when searching for donors:
            - "distance": by normalized inverse distance to the target (default).
            - "random": within each nested group (if id_nested is given), candidates are
                        randomly permuted. If id_nested is None, all candidates are shuffled.
            If None, falls back to self.candidate_sampling.
        max_candidate_pool : int or None, optional
            If not None, only the closest `max_candidate_pool` candidate series (after
            ordering) are tested. If None, falls back to self.max_candidate_pool.
        ...
        for i in tqdm(np.unique(unique_int), desc="STEP 3. Spatiotemporal filling"):
            ...
            for k in cand_ids:
                ...
                if corr_temp >= self.threshold_corr:
                    ...
                    # update per-nested count
                    if (nested_by_id is not None) and (self.n_candidate_nested is not None):
                        seg_k = nested_by_id[k]
                        nested_counts[seg_k] = nested_counts.get(seg_k, 0) + 1

                # Optional global candidate limit for speed
